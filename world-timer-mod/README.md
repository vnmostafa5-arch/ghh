...
- ⏱ Tracks time spent in each world using server ticks (20 ticks = 1 second)
...
Download the built artifact from the **Actions** tab → select a workflow run → download `worldtimer-mod`.

## How It Works

1. When you open a world, the timer starts counting
2. Time is displayed on the HUD as `⏱ HH:MM:SS`
3. When you leave the world, the time is saved to a JSON file
